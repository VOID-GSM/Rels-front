...
export default function ThisWeekPage() {
  ...
  return (
    <PageShell size="narrow">
      {/* 고를 강연은 스포트라이트 옆에 세워 둡니다. 위에 얹으면 강연 제목보다 먼저
          읽히고, 아래로 스크롤하면 사라져 다른 강연으로 가려면 다시 올라와야 합니다.
          다만 스포트라이트를 좁히면서까지 자리를 만들지는 않습니다. 본문은 그대로 두고
          PageShell(1080px) 바깥에 이미 비어 있는 여백에만 얹습니다. 본문 952px 기준으로
          한쪽 여백이 264px(칸 240 + 간격 24)을 넘기는 1520px부터가 그 조건입니다.
          그보다 좁으면 놓을 여백이 없으므로 스포트라이트 위로 올라갑니다. */}
      <div className="relative mt-6 md:mt-12">
        {hasMultipleLive && (
          <aside className="mb-8 min-[1520px]:absolute min-[1520px]:inset-y-0 min-[1520px]:left-full min-[1520px]:mb-0 min-[1520px]:ml-6 min-[1520px]:w-60">
            {/* 헤더가 sticky top-0(68px)이라 그 아래에 붙여야 가려지지 않습니다.
                긴 본문을 내려도 옆 칸은 따라옵니다. */}
            <div className="min-[1520px]:sticky min-[1520px]:top-[84px]">
              <LectureSwitcher
                items={switcherItems}
